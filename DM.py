...
class DM(object):
    ...
    def __init__ (self, soapy_config, n_dm=0, wfss=None, mask=None):
        ...
        if(self.soapy_config.tel.telDiam==0):
            if(self.soapy_config.tel.telLen>=self.soapy_config.tel.telWidth):
                self.telescope_diameter=self.soapy_config.tel.telLen
            else:
                self.telescope_diameter=self.soapy_config.tel.telWidth
        else:
...
class TT(DM):
    ...
    def makeIMatShapes(self):
        ...
        if(self.soapy_config.tel.telDiam==0):
        	if(self.soapy_config.tel.telLen>=self.soapy_config.tel.telWidth):
               		tt_amp = -(ASEC2RAD * self.soapy_config.tel.telLen/2.) * 1e9
        	else:
                	tt_amp = -(ASEC2RAD * self.soapy_config.tel.telWidth/2.) * 1e9
        else:
