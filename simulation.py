...
class Sim(object):
    ...
    def makeSaveHeader(self):
        ...
        # Tel Params
        header["MASK"] = self.config.tel.mask
        header["TELWIDTH"] = self.config.tel.telWidth
        header["TELLEN"] = self.config.tel.telLen
...
def make_mask(config):
    ...
    elif config.tel.mask == "rectangle":
        mask = aotools.rectangle(config.tel.telLen, config.tel.telWidth,config.sim.simSize)
